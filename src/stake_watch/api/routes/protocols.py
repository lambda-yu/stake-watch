...
async def refresh_all_protocols(
    ...
    for p in protos:
        ...
        # Per-family refresher (Morpho/Kamino/Jupiter/Aave/Compound/Sky).
        from stake_watch.collectors.refreshers import pick_refresher
        pick = pick_refresher(p)
        if pick:
            source, refresher = pick
            try:
                entry = await refresher(p, store, storage)
                if entry:
                    refreshed.append(entry)
                    continue
            except Exception as e:
                failed.append({"name": p.name,
                               "reason": f"{source}: {type(e).__name__}: {e}"})
                continue
